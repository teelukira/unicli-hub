...
Append to audit.md. Read `.unicli-rules/construction/build-and-test.md`.
...
Task({
  ...
  prompt: "You are the tmf-compliance-reviewer agent. Perform full TMF-A through TMF-N (14 rules) verification for unit {unit-name}. Scope: [TMF639/634/641/702 as applicable]. Reference: specs/tmf/ directory. Use atom-tmf-kb-mcp tools. Output: PASS, CONDITIONAL PASS, or FAIL with specific rule violations.",
  subagent_type: "tmf-compliance-reviewer"
})
```

Log invocation to audit.md. Write `GATE-TMF: PASS | CONDITIONAL PASS | FAIL` to audit.md.
...
- `GATE-TMF: PASS | CONDITIONAL PASS | N/A` required
...
## Step 10: Jira Done transition (MANDATORY)

Read `common/jira-integration.md` §3a. Read unit's `**Jira Ticket**` entries from `aidlc-docs/aidlc-state.md`.
Transition each ticket to Done (`transition_id_done: "111"`).
Log `JIRA-RESOLVED: NWAE-### [unit={unit}]` to audit.md.
...
**Suggested MR title**: feat({unit-id}): {unit-title} — {ticket-keys}
