...
### Step 2.5: codegen-iac IaC Blueprint Validation (CONDITIONAL)

**Condition**: Invoke when infrastructure design includes new AWS/cloud resource types not previously defined in this project, or when IaC patterns need validation against the existing Terraform module structure under `infra/`. Skip when the infrastructure follows an already-established pattern with no new resource types.

**Action**: Use the Agent tool with `subagent_type: "codegen-iac"` to validate the planned infrastructure design against current AWS best practices and the existing `infra/` Terraform module structure, and to produce a draft IaC blueprint.

- **Inputs**: Infrastructure design plan path, `infra/` directory listing, list of new resource types to introduce.
- **Blocking**: Yes — wait for subagent completion before proceeding to Step 3.
- **On failure**: Surface IaC validation errors to user; do not silently skip.
- **Audit**: Append to `aidlc-docs/audit.md`:
  ```
  ## Subagent Invocation — codegen-iac
  **Timestamp**: [ISO 8601]
  **Stage**: Infrastructure Design
  **Unit**: [unit-name]
  **Purpose**: IaC blueprint validation and pattern check
  **Result**: [summary]
  ---
  ```

If condition is not met: log `"codegen-iac N/A — follows existing infra/ pattern"` in audit.md and proceed directly to Step 3.
