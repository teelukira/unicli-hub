...
Functional design refines an existing application design and typically does NOT introduce new architectural decisions. However:

1. **Consult existing ADRs**: Before drafting business logic, read related ADRs (use [`aidlc-docs/index/adr-index.md`](../../aidlc-docs/index/adr-index.md) "Affects-Units 역참조" to find ADRs relevant to this unit).
2. **Cite in unit INDEX**: Update `aidlc-docs/construction/{unit}/INDEX.md` top `**Relates-To-ADR**:` line if new ADRs apply.
3. **Escalate violations**: If functional design forces a deviation from an `Accepted` ADR (e.g., business logic requires breaking hexagonal layering), STOP — escalate to `adr-curator` for new ADR (Supersede) before continuing.
