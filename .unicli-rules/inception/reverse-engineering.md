...
## Step 0.5: aidlc-researcher Pre-Research (CONDITIONAL)

**Condition**: Invoke ONLY when encountering an unfamiliar technology stack, library, or architectural pattern during initial workspace scan. Skip if all components are already well-understood.

**Action**: Use the Agent tool with `subagent_type: "aidlc-researcher"` to research the technology stack and design patterns before generating reverse engineering artifacts.

- **Inputs**: List of unfamiliar components/frameworks identified, research questions.
...
- **Audit**: Append to `aidlc-docs/audit.md`:
  ...
  **Purpose**: Technology stack pre-research
  **Result**: [summary]
  ---
  ```

If condition is not met: log `"aidlc-researcher N/A — all tech stack components familiar"` in audit.md and proceed directly to Step 1.
