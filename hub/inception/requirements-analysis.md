...
### Step 0.5: TMF Knowledge Oracle Warm-Up (CONDITIONAL)

**Condition**: Invoke ONLY if BOTH hold:
1. This is a TMF-standard unit (the unit will implement a TMF Open API specification).
2. Any file under `docs/raw/` has mtime newer than the newest file under `docs/tmf-oracle/`, OR `docs/tmf-oracle/` is empty.

**Action**: Use the Agent tool with `subagent_type: "tmf-knowledge-ingest"` to parse new/updated raw TMF assets (PDF/Excel/XMI under `docs/raw/`) and populate `docs/tmf-oracle/` before requirements analysis.

- **Inputs**: Path to `docs/raw/` and `docs/tmf-oracle/`, list of unparsed files.
- **Blocking**: Yes — wait for subagent completion before proceeding to Step 1.
- **On failure**: Surface error to user; do not silently skip.
- **Audit**: Append to `aidlc-docs/audit.md`:
  ```
  ## Subagent Invocation — tmf-knowledge-ingest
  **Timestamp**: [ISO 8601]
  **Stage**: Requirements Analysis
  **Purpose**: TMF Knowledge Oracle warm-up
  **Result**: [summary]
  ---
  ```

If condition is not met: log `"tmf-knowledge-ingest N/A — oracle current or non-TMF unit"` in audit.md and proceed directly to Step 1.
