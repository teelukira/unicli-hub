...
- [ ] 키가 비어 있거나 placeholder만 있으면: **Create Story** 절차 전체 실행
  ...
  - [ ] Summary: `[{unit-name}] {간결한 기능 제목}` — 기능·개선점 중심, ~50자 권장
  - [ ] Description: `scripts/render-ai-dlc-remote-templates.py`로 `hub/templates/remotes/jira-issue.body.md` 렌더 (requirements / execution-plan / code-generation-plan 경로, `approved_at`, reporter, assignee 값 JSON으로 전달)
