...
## Step 4.5: tmf-compliance-reviewer Subagent (3-Pillar & MCP Integration) (CONDITIONAL — BLOCKING if mandatory)
...
1. Use the Agent tool with `subagent_type: "tmf-compliance-reviewer"` for this unit:
   ...
   - **Verification Strategy (The 3-Pillar Architecture)**:
     - **Pillar 1: TMF API Spec Conformance**: Run or analyze `scripts/validate-api-contract.sh --service <service> --mode all` results, check Spectral linting errors (must be 0), and check Controller-to-Architect spec diff.
     - **Pillar 2: SID Schema Conformance**: Run or analyze `python3 scripts/validate-sid-schema.py --unit <unit>` and `python3 scripts/validate-seed-rules.py --service <service> --mode static` results, mapping Flyway DDL tables/columns to SID ABE classes/JPA attributes.
     - **Pillar 3: CTK Conformance**: Review standard `specs/tmf/ctk/ctk-status-report.md` and Newman assertions. Verify standard lifecycle transitions and event publishers.
     - **Knowledge-Base Cross-Reference**: Initialize `atom-tmf-kb-mcp` tools by running `tmf_kb_list_releases()` and pinning the target version via `tmf_kb_pin({version})`. Use asset, landscape, and scenario retrievals to troubleshoot Pillar errors, and invoke `tmf_kb_judge` for 4-axis unified conformance verdict.
   - **Audit** — append to `aidlc-docs/audit.md` after completion:
     ```
     ...
     **Stage**: Build and Test
     **Unit**: [unit-name]
     **Purpose**: TMF-A~N 3-Pillar compliance verification using atom-tmf-kb-mcp
     **Result**: [PASS / FAIL]
     ---
     ```

2. **Expected outputs** (verify existence):
   - `specs/tmf/{unit}/review-report.md` — verdict MUST be `PASS` (No CONDITIONAL PASS allowed)
   - `specs/tmf/{unit}/compliance-evidence.md` — containing MCP quotes and automated script logs
   - `specs/tmf/{unit}/component-mapping.md` — defining ODA component map
...
2. **The agent runs three phases**:
   ...
   - Phase 2: Local Playwright E2E — Playwright `webServer` auto-spawns backends (bootJar → java -jar → Vite dev). Prerequisites: `frontend/.env.local` has `VITE_OIDC_ENABLED=false`, `playwright.config.ts` has `webServer` array, all API paths proxied in `vite.config.ts`. **`E2E_FRONTEND_ONLY=true` 환경변수가 설정된 실행은 이 BLOCKING 게이트를 충족하지 않음 — 반드시 실 백엔드와 함께 실행해야 함.**
...
- **Verdict**: [PASS/FAIL/N/A] (No CONDITIONAL PASS allowed)
...
- `tmf-compliance-reviewer` verdict = PASS or N/A (Strict PASS only, no CONDITIONAL PASS)
