...
This file is the **slim always-on** Cursor rule. Full prose for every stage lives under `hub/` and is loaded **on demand** inside skills — do not duplicate that material here.
...
3. **Skill index & architecture**: `hub/README.md`
4. **Full cross-CLI workflow reference** (read when you need the complete stage list or governance text): `hub/core-workflow.md`

## Rule details directory

All stage-specific instructions live under `hub/` (see `common/process-overview.md`, `inception/*.md`, `construction/*.md`, `extensions/**`).
...
Canonical procedure: `hub/common/jira-integration.md`, `hub/templates/remotes/README.md`, and Construction `aidlc-build-and-test` Step 11. Cursor hooks enforce markers; do not call remote MCP until audit/state match the hook contract.
...
### Git branch discipline (before any MR)

SSOT: `common/git-worktree-conventions.md`, `docs/conventions/gitlab-mr-workflow.md`.

- **Do not commit on `main`** for feature/fix work — use `{type}/{branch}` or a git worktree.
- **Do not** run `git merge --no-ff <feature>` on local `main` then open an MR (GitLab shows **no changes**).
- Before `gitlab_create_merge_request`, verify `git log origin/main..HEAD` is non-empty and review `git diff --stat origin/main...HEAD`.
- After GitLab merges the MR, sync with **`git checkout main && git pull origin main` only** — do not describe this as “local merge”; do not run `git merge` on `main` for the same change.
...
4. MR **title** is a concise feature-focused summary (no Jira keys). Jira keys are included in the MR **body** (`{jira_keys}` rendered via `--target gitlab-body`), per `common/jira-integration.md`.
...
**Hook stdout contract**: `failClosed` hooks must print `{"permission":"allow"}` on stdout whenever the tool is allowed. Empty stdout on exit 0 is treated as hook failure and blocks the MCP call (see `common/jira-integration.md` §2-4).

## Build and Test (Cursor — strict gates + codegen feedback)

- Before claiming **Build and Test** complete or writing `GATE-QA: PASS` in `aidlc-docs/audit.md`, read the **entire** `.cursor/skills/aidlc-build-and-test/SKILL.md` and follow Steps 3–7 (and `hub/construction/build-and-test.md`).
